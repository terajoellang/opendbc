...
HOLD       – driver holds wheel, gentle nudge ≤ 4 °/frame  
...
* Grace window (0.4 s) fires only on **manual** AP enable and now sends the
  *planner angle immediately* (no ramp) while still ignoring driver torque
  < 5 Nm. Emergency breakout ≥ 5 Nm works during grace.
* Ramp durations ≥ 5 km/h = 1 s. Stand-still remains 1 s.
...
TORQUE_EXIT       = 0.8                 # driver released ≤ (user pref)
...
class TorqueBlendingCarController:
    ...
    @staticmethod
    def _tau_for_speed(v: float) -> float:
        if v < V_5:
            return TAU_STAND
        if v < V_10:
            return TAU_5_10
    ...
    def update_torque_blending(
        ...
        angle_delta = abs(apply_angle - wheel_angle)

        # Detect manual enable rising edge → start grace window
        if lat_active and not self._prev_lat_active:
            self.grace = GRACE_TIME
            # reset FSM to AUTO so we send planner angle immediately
            self.state = TBState.AUTO
        ...
        ignore_small_torque = in_grace and abs(driver_torque) < GRACE_BREAK

        # Dynamic enter threshold (easier in curves)
        dyn_enter = max(TORQUE_ENTER_BASE - TORQUE_ANGLE_SCALE * angle_delta, TORQUE_ENTER_MIN)
        ...
        if ignore_small_torque:
            ...
            self._above_t = self._above_t + self.dt if abs(driver_torque) >= dyn_enter else 0.0
            self._below_t = self._below_t + self.dt if abs(driver_torque) <= TORQUE_EXIT else 0.0

        # --------------- FSM transitions -----------
        if self.state == TBState.AUTO and not in_grace:
            if self._above_t >= ENTER_TIME:
                self.state = TBState.HOLD

        elif self.state == TBState.HOLD:
            if (angle_delta < ANGLE_MATCH and self._below_t >= EXIT_TIME) or self._below_t >= 2 * EXIT_TIME:
